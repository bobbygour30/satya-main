...
export default function CaseStudyEditor({ caseStudy, onBack, onSave }) {
  ...
  const [formData, setFormData] = useState({
    ...
    content: [
      ...
      // ... rest of your content array (truncated for brevity)
    ],
  ...
  const handleHeroUpload = async (type, file, index) => {
    if (!caseStudy?._id) {
      setSaveStatus({ type: 'error', message: 'Please save the case study first' });
      return;
    }
  ...
  const handleHeroDelete = async (type, imageId, index) => {
    try {
      ...
      setFormData(prev => {
        const newImages = [...(prev.heroImages[type] || [])];
        newImages[index] = null; // Set to null instead of filtering to maintain indices
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600 text-sm mt-1">{formData.name || 'Untitled'}</p>
          </div>
        </div>
              ...
              <div className="space-y-8">
                ...
                <div>
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-semibold text-[#9E4A47]">Before Surgery Images</h3>
                    <p className="text-sm text-gray-500">Upload 4 images (Front, Left, Right, Back views)</p>
                  </div>
                      ...
                      return (
                        <div key={`before-${idx}`} className="space-y-2">
                          <ImageUploader
                            ...
                            onRemove={image ? () => handleHeroDelete('before', image._id, idx) : undefined}
                            uploading={uploading}
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              if (image) {
                                ...
                                newImages[idx] = { ...image, name: e.target.value };
                                setFormData(prev => ({
                                  ...prev,
                                  heroImages: {
                                    ...prev.heroImages,
                ...
                <div>
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-semibold text-[#9E4A47]">After Surgery Images</h3>
                    <p className="text-sm text-gray-500">Upload 4 images (Front, Left, Right, Back views)</p>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[0, 1, 2, 3].map((idx) => {
                      ...
                      return (
                        <div key={`after-${idx}`} className="space-y-2">
                          <ImageUploader
                            ...
                            onRemove={image ? () => handleHeroDelete('after', image._id, idx) : undefined}
                            uploading={uploading}
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-700">
                  ...
                  <p>The first image from Before and After sets will be shown in the hero slider. Users can navigate through all 4 views using the arrows.</p>
                </div>
              </div>
            )}
            ...
            {activeTab === 'details' && (
              <div className="space-y-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-lg font-semibold text-[#2B333C]">Patient Information</h3>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setFormData({
                          ...formData,
                          patientDetails: { ...formData.patientDetails, name: e.target.value }
                        })}
                        className="w-full p-2 border rounded-lg"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setFormData({
                          ...formData,
                          patientDetails: { ...formData.patientDetails, surgeryDate: e.target.value }
                        })}
                        className="w-full p-2 border rounded-lg"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Follow-up Period</label>
                      <input
                        type="text"
                        value={formData.patientDetails.followUp}
                        onChange={(e) => setFormData({
                          ...formData,
                          patientDetails: { ...formData.patientDetails, followUp: e.target.value }
                        })}
                        className="w-full p-2 border rounded-lg"
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Donor Area</label>
                      <input
                        type="text"
                        value={formData.patientDetails.donorArea}
                        onChange={(e) => setFormData({
                          ...formData,
                          patientDetails: { ...formData.patientDetails, donorArea: e.target.value }
                        })}
                        className="w-full p-2 border rounded-lg"
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) => setFormData({
                          ...formData,
                          patientDetails: { ...formData.patientDetails, complications: e.target.value }
                        })}
                        className="w-full p-2 border rounded-lg"
                        rows="3"
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                          ...
                          <div key={idx} className="flex gap-2">
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                setFormData({
                                  ...formData,
                                  patientDetails: { ...formData.patientDetails, medications: newMeds }
                                });
                              }}
                            ...
                            <button
                              onClick={() => {
                                const newMeds = formData.patientDetails.medications.filter((_, i) => i !== idx);
                                setFormData({
                                  ...formData,
                                  patientDetails: { ...formData.patientDetails, medications: newMeds }
                                });
                        ...
                        <button
                          onClick={() => {
                            const newMeds = [...(formData.patientDetails.medications || []), 'New medication'];
                            setFormData({
                              ...formData,
                              patientDetails: { ...formData.patientDetails, medications: newMeds }
                            });
                ...
                <div className="border-t pt-6 mt-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">1st Surgery Grafts</label>
                      <input
                        type="text"
                        ...
                        onChange={(e) => setFormData({
                          ...
                          patientDetails: {
                            ...
                            grafts: { ...formData.patientDetails.grafts, first: e.target.value }
                          }
                        })}
                        className="w-full p-2 border rounded-lg"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">2nd Surgery Grafts</label>
                      <input
                        type="text"
                        ...
                        onChange={(e) => setFormData({
                          ...
                          patientDetails: {
                            ...formData.patientDetails,
                            grafts: { ...formData.patientDetails.grafts, second: e.target.value }
                          }
                        })}
                        className="w-full p-2 border rounded-lg"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setFormData({
                          ...formData,
                          patientDetails: { ...formData.patientDetails, technique: e.target.value }
                        })}
                        className="w-full p-2 border rounded-lg"
                        placeholder="e.g., MHT"
                      />
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Technique Detail</label>
                      <input
                        type="text"
                        value={formData.patientDetails.techniqueDetail}
                        onChange={(e) => setFormData({
                          ...formData,
                          patientDetails: { ...formData.patientDetails, techniqueDetail: e.target.value }
                        })}
                        className="w-full p-2 border rounded-lg"
                ...
                <div className="border-t pt-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setFormData({
                        ...formData,
                        patientDetails: { ...formData.patientDetails, clinic: e.target.value }
                      })}
                      className="w-full p-2 border rounded-lg"
            ...
            {activeTab === 'table' && (
              <div className="space-y-6">
                          ...
                          <th key={col.id} className="p-3 border min-w-[180px]">
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                setFormData({
                                  ...formData,
                                  tableData: { ...formData.tableData, columns: newCols }
                                });
                              }}
                            ...
                            return (
                              <td key={col.id} className="p-2 border">
                                <ImageUploader
                                  ...
                                  currentPreview={image?.url}
                                  onUpload={(file) => handleTableImageUpload(row.id, col.id, file)}
                                  onRemove={image ? () => {
                                    // Handle delete
                                  } : undefined}
                                  uploading={uploading}
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-sm text-yellow-700">
                  ...
                  <p>Each cell represents a specific view at a specific stage. Upload images for each combination to show the patient's progress.</p>
                </div>
              </div>
            )}
            ...
            {activeTab === 'content' && (
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  <h3 className="text-lg font-semibold text-[#2B333C]">Content Sections</h3>
